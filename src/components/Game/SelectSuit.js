...
const SelectSuit = (props) => {
  ...
  const [selectedSuit, updateSelectedSuit] = useState({
    suit: "",
    possibleIssue: false,
  })
  ...
  const selectFromDummy = (suit) => (e) => {
    ...
    if (
      ...
      dispatch({
        ...
        payload: { type: "warning", message: "Please select a suit!" },
      })
      return
    }
  ...
  const riskOfMistakeBuyingCards = (suit) => {
    let deletingCards = removeAllFromArray(
      selectedCards.map((sc) => sc.card),
      game.cards
    )
    ...
    for (var i = 0; i < deletingCards.length; i++) {
      if (
        deletingCards[i] === "JOKER" ||
        deletingCards[i] === "ACE_HEARTS" ||
        deletingCards[i].split("_")[1] === suit
      ) {
        return true
      }
    }
            ...
            <div>
              <ButtonGroup size="lg">
                <Button
                  type="button"
                  color="secondary"
                  onClick={selectFromDummy("HEARTS")}
                >
                  <img
                    alt="Hearts"
                ...
                <Button
                  type="button"
                  color="secondary"
                  onClick={selectFromDummy("DIAMONDS")}
                >
                  <img
                    alt="Hearts"
                ...
                <Button
                  type="button"
                  color="secondary"
                  onClick={selectFromDummy("SPADES")}
                >
                  <img
                    alt="Hearts"
              ...
              {selectedSuit.possibleIssue ? (
                <Modal
                  ...
                  isOpen={selectedSuit.possibleIssue}
                >
                  <ModalHeader>
                    <CardImg
                      alt="Suit"
                      src={`/cards/originals/${selectedSuit.suit}_ICON.svg`}
                        ...
                        <CardBody className="buttonArea">
                          <ButtonGroup size="lg">
                            ...
                            <Button
                              type="button"
                              color="warning"
                              onClick={submitSelectFromDummyEvent(
                                selectedSuit.suit
                              )}
                            >
                              Throw Cards

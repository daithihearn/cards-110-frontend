...
const bestCardLead = (round) => {
  let trumpCards = cards.filter(
    (card) => card.suit === round.suit || card.suit === "WILD"
  )
...
const AutoActionManager = (props) => {
  ...
  const autoPlayCards = orderedCards.filter(
  ...
  const buyCards = (cardsToBuy) => {
    getAccessTokenSilently()
      .then((accessToken) => {
        GameService.buyCards(game.id, cardsToBuy, accessToken)
          .then((response) => {
  ...
  const chooseWorstCardToFollow = (myCards, suit) => {
    ...
    const myTrumpCards = myCardsRich.filter(
      (card) => card.suit === suit || card.suit === "WILD"
    )

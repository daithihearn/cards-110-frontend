          ...
          {!!game.round.suit ? (
            <Col xs="9">
              <h2 className="cardAreaHeader">
                <CardImg
                  alt={players.find((p) => p.id === game.round.goerId).name}
                  src={players.find((q) => q.id === game.round.goerId).picture}
                  className="thumbnail_size_extra_small"
                />
                ...
                <CardImg
                  ...
                  src={`/cards/originals/call_${
                    game.playerProfiles.filter(
                      (profile) => profile.id === game.round.goerId
                    )[0].call
                  }.png`}

        ...
        <Row>
          {game.playerProfiles.sort(compareSeat).map((playerProfile, idx) => (
            <Col key={`cards_${idx}`} className="player-column">
              <div>
                <CardImg
                  alt={players.find((p) => p.id === playerProfile.id).name}
                  src={players.find((q) => q.id === playerProfile.id).picture}
                  className="avatar"
                />
                <CardImgOverlay>
                ...
                {!!game.round.currentHand.playedCards[playerProfile.id] ? (
                  ...
                  <a>
                    <CardImg
                      alt={playerProfile.displayName}
                      src={`/cards/thumbnails/${
                        game.round.currentHand.currentPlayerId ===
                        playerProfile.id
                          ? "yellow"
                          : "blank_grey"
                      }_back.png`}
                    ...
                    {!game.round.suit ? (
                      <CardImgOverlay>
                        ...
                        {playerProfile.call === 30 &&
                        game.playerProfiles.length === 2 ? (
                          <CardImg
                            alt="30 Chip"

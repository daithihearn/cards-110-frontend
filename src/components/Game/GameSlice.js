...
const processOrderedCards = (state, action) => {
  ...
  const delta = !!state.game.cards
    ? state.game.cards.filter((x) => !action.payload.cards.includes(x))
    : []
  ...
  // 2. If a card was removed then replace it with a Blank card in orderedCards
    ...
    !!state.game.cards &&
    state.game.cards.length === action.payload.cards.length + 1 &&
    delta.length === 1
  ) {
    ...
    action.payload.cards.forEach((c) =>
      orderedCards.push({ card: c, autoplay: false, selected: false })
    )
  }
  return orderedCards
}

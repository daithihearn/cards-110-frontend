...
const MyCards = (props) => {
  ...
  const playButtonEnabled =
    ...
    game.round.completedHands.length + game.cards.length === 5

  const [doubleClickTracker, updateDoubleClickTracker] = useState({})

  const BLANK = "blank_card"
  const cardsSelectable = ["CALLED", "BUYING", "PLAYING"].includes(
    game.round.status
  )
    ...
    if (selectedCards.length !== 1) {
      dispatch({
        ...
        payload: {
          type: "error",
          message: parseError({
            message: "Please select exactly one card to play",
          }),
        },
      })
    } else {
                ...
                >
                  {orderedCards.map((card, index) => {
                    return (
                      <Draggable
                        key={card.card + index}
                        draggableId={card.card + index}
                        index={index}
                        isDragDisabled={card.card === BLANK}
                      >
                        {(provided) => (
                          <div
                            ref={provided.innerRef}

  ...
  const columns = [
    {
      ...
      cell: (row) => (
        <div>
          <img
            alt="Image Preview"
            src={players.find((p) => p.id === row.player1.id).picture}
            className="avatar"
          />
          ...
          {!gameOver && !!previousHand ? (
            <div>
              {!!previousHand && !!previousHand.playedCards[row.player1.id] ? (
                <img
                  alt={previousHand.playedCards[row.player1.id]}
                  src={
                    "/cards/thumbnails/" +
                    previousHand.playedCards[row.player1.id] +
                    ".png"
                  }
    ...
    },
    {
      ...
      cell: (row) => (
        <div>
          <img
            alt="Image Preview"
            src={players.find((p) => p.id === row.player2.id).picture}
            className="avatar"
          />
          ...
          {!gameOver && !!previousHand ? (
            <div>
              {!!previousHand && !!previousHand.playedCards[row.player2.id] ? (
                <img
                  alt={previousHand.playedCards[row.player2.id]}
                  src={
                    "/cards/thumbnails/" +
                    previousHand.playedCards[row.player2.id] +
                    ".png"
                  }
                  className="thumbnail_size_small cardNotSelected"
                />
